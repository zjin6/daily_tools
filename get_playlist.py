...
@retry(max_attempts=None, sleep_time=0)
def get_playlist_links(youtube_url):
    response = requests.get(youtube_url)
    soup = BeautifulSoup(response.text, 'html.parser')
    playlist_links = []

    for link in soup.find_all('a'):
        if link.text.strip() == 'View full playlist':
            playlist_links.append(link['href'])
...
if __name__ == '__main__':
    ...
    youtube_url = input("channel url: ")
    # get_playlist_url(channel_url)
    # get_playlist(channel_url)

    # youtube_url = 'https://www.youtube.com/your_video_page_url'
    playlist_links = get_playlist_links(youtube_url)
